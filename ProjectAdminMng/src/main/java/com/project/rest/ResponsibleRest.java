...
public class ResponsibleRest {
	...
	int id, Responsible responsible) throws ProjectException {
		try {
			Activity activity = new Activity();
			activity.setId(id);

			this.responsibleService.update(activity, responsible);
			...
			ex.printStackTrace();
			throw new ProjectException("Error actualizando el responsable");
		}
